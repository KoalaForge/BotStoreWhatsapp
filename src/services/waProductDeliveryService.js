...
async function deliverProductToCustomer(sock, jid, context, {
    ...
    const shouldSendAsFile = fileDelivery.shouldDeliverAsFile(orderQuantity);
    ...
    if (shouldSendAsFile) {
        ...
        // QUANTITY <= 5: Send as regular message

        const message = waMessageFormatter.formatProductSuccessMessage({
            transactionId,
            productName,
            variantName,
            orderQuantity,
        ...
        await withRetry(() => sock.sendMessage(jid, { text: message }));
