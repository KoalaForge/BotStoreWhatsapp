...
class QRISService {
     ...
     * @param {Object|null} ctx - WaCtx context (for ownerId in MULTI mode)
     ...
     * @param {boolean} honorFeePaidBy - Whether to honor /setfee setting
     ...
     * @param {Object} params - Parameters for top-up
     * @param {Object} params.ctx - WaCtx context
     * @param {number} params.nominal - Top-up amount (before fee)
     * @returns {Promise<Object>}
     */
    async generateTopUpQRIS({ ctx, nominal }) {
     ...
     */
    async generateOwnerTopUpQRIS({ ctx, nominal }) {
        const userId = ctx.from;
        const uniqCode = await transactionService.generateTransactionId('topup', ctx);
     ...
     */
    async generateProductQRIS({ ctx, totalAmount, orderAmount, price, productName, fee = 0, voucherCode = null, voucherDiscount = 0 }) {
        const userId = ctx.from;
     ...
     * @param {string} [params.ownerId] - Owner ID for MULTI mode gateway resolution
